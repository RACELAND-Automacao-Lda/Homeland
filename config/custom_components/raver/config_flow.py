...
class FlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_mqtt(self, discovery_info: MqttServiceInfo) -> FlowResult:
        ...
        if not discovery_info.topic.endswith("/config"):
            # Not a Raver discovery message
            return self.async_abort(reason="invalid_discovery_info")
        if not discovery_info.payload:
            # Empty payload, the Raver is not configured for native discovery
            return self.async_abort(reason="invalid_discovery_info")

        # "raver/discovery/#" is hardcoded in Raver's manifest
        assert discovery_info.subscribed_topic == "raver/discovery/#"
        self._prefix = "raver/discovery"
    ...
    async def async_step_config(
        ...
        if user_input is not None:
            ...
            if not bad_prefix:
                return self.async_create_entry(title="Raver", data=data)
