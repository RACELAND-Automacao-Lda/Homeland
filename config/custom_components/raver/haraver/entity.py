...
class RaverEntityConfig:
    """Base class for Raver configuation."""
...
@attr.s(slots=True, frozen=True)
class RaverAvailabilityConfig(RaverEntityConfig):
    """Raver availability configuation."""
...
class RaverEntity:
    ...
    def __init__(self, config: RaverEntityConfig, mqtt_client: RaverMQTTClient):
        """Initialize."""
        self._cfg = config
    ...
    def config_same(self, new_config: RaverEntityConfig) -> bool:
        """Return if updated config is same as current config."""
        return self._cfg == new_config

    def config_update(self, new_config: RaverEntityConfig) -> None:
        """Update config."""
...
class RaverAvailability(RaverEntity):
    """Availability mixin for Raver entities."""

    _cfg: RaverAvailabilityConfig
