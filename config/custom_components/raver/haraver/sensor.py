...
from .entity import RaverAvailability, RaverEntity
from .models import DiscoveryHashType, RaverBaseSensorConfig
...
class RaverSensorConfig(RaverBaseSensorConfig):
    """Raver Status Sensor configuration."""
...
class RaverSensor(RaverAvailability, RaverEntity):
    """Representation of Raver Status Sensors."""

    _cfg: RaverSensorConfig
...
def _get_sensor_entity(
    ...
    parent_path: list[str | int],
    quantity: str,
) -> tuple[RaverSensorConfig, DiscoveryHashType]:
    ...
    sensor_config = RaverSensorConfig.from_discovery_message(
...
) -> list[tuple[RaverBaseSensorConfig, DiscoveryHashType]]:
    """Generate sensor configuration."""
    sensor_configs: list[tuple[RaverBaseSensorConfig, DiscoveryHashType]] = []
    for sensorkey, sensor in sensor_discovery_message[CONF_SENSOR].items():
        ...
        for subsensorkey, subsensor in sensor.items():
            ...
            if isinstance(subsensor, dict):
                ...
                # Simple sensor
                if (not bool(re.match("PCF", subsensorpath[0]))): 
                    sensor_configs.append(
                        _get_sensor_entity(
                            sensor_discovery_message,
                            device_discovery_msg,
