...
class RaverLightConfig(RaverAvailabilityConfig, RaverEntityConfig):
    """Raver light configuation."""
    ...
    def from_discovery_message(
        ...
        raver_light_sub_type = config[CONF_LIGHT_SUBTYPE]
        light_type = LIGHT_TYPE_MAP[raver_light_sub_type]
        ...
        if config[CONF_OPTIONS][OPTION_PWM_MULTI_CHANNELS]:
        ...
        elif not config[CONF_LINK_RGB_CT] and raver_light_sub_type >= LST_RGBW:
            ...
            if idx - first_light == 1:
                ...
                if raver_light_sub_type == LST_RGBW:
                    light_type = LIGHT_TYPE_DIMMER
                else:
...
class RaverLight(RaverAvailability, RaverEntity):
    """Representation of a Raver light."""

    _cfg: RaverLightConfig
    ...
    def _calculate_speed(self, state: bool, attributes: dict[str, Any]) -> float:
        ...
        # Raver's speed command is the number of half-seconds, scaled for a 100% fade
