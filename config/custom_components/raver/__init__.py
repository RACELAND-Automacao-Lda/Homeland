"""The Raver integration."""
from __future__ import annotations

import asyncio
import logging

from .haraver.const import (
    CONF_IP,
    CONF_MAC,
    CONF_MANUFACTURER,
    CONF_MODEL,
...
from .haraver.discovery import clear_discovery_topic
from .haraver.models import RaverDeviceConfig
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Raver from a config entry."""
    ...
    raver_mqtt = RaverMQTTClient(_publish, _subscribe_topics, _unsubscribe_topics)

    device_registry = dr.async_get(hass)

    async def async_discover_device(config: RaverDeviceConfig, mac: str) -> None:
        """Discover and add a Raver device."""
        await async_setup_device(
            hass, mac, config, entry, raver_mqtt, device_registry
        )
    ...
    async def async_device_updated(event: Event) -> None:
        ...
        for mac in macs:
            await clear_discovery_topic(
                mac, entry.data[CONF_DISCOVERY_PREFIX], raver_mqtt
            )
    ...
    async def start_platforms() -> None:
        ...
        await discovery.async_start(
            hass, discovery_prefix, entry, raver_mqtt, async_discover_device
        )
...
async def _remove_device(
    ...
    _LOGGER.debug("Removing raver from device %s", mac)
    device_registry.async_update_device(
        device.id, remove_config_entry_id=config_entry.entry_id
    )
    await clear_discovery_topic(
        mac, config_entry.data[CONF_DISCOVERY_PREFIX], raver_mqtt
    )
...
def _update_device(
    hass: HomeAssistant,
    config_entry: ConfigEntry,
    config: RaverDeviceConfig,
    device_registry: DeviceRegistry,
) -> None:
    """Add or update device registry."""
    _LOGGER.debug("Adding or updating raver device %s", config[CONF_MAC])
...
async def async_setup_device(
    ...
    """Set up the Raver device."""
    if not config:
        await _remove_device(hass, config_entry, mac, raver_mqtt, device_registry)
    else:
        _update_device(hass, config_entry, config, device_registry)


@websocket_api.websocket_command(
    {vol.Required("type"): "raver/device/remove", vol.Required("device_id"): str}
)
@callback
def websocket_remove_device(
    ...
    for config_entry_id in device.config_entries:
        ...
        assert config_entry
        # Only delete the device if it belongs to a Raver device entry
    ...
    connection.send_error(
        msg["id"], websocket_api.const.ERR_NOT_FOUND, "Non Raver device"
    )
...
async def async_remove_config_entry_device(
    ...
    """Remove Raver config entry from a device."""
